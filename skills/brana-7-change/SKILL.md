...
- **Stale-plan rule (Phase 5) applies here too:** a mid-cycle PLAN.md section rewrite reverts PLAN.md and TASKS.md to `draft` until the scoped re-gate is clean — consistency checks on the patched section, `tools/brana-gate tasks` on the tasks serving the patched chunk — before the next implementation session.
