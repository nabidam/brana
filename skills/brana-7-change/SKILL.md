...
| **C — big feature** | New module, schema migration, new integration | Full Phase 1→6 scoped to the delta (`brana-1` … `brana-6`), new `specs/NNN-name/` dir. Phase 1 runs the **Route C delta qualification** (WORKFLOW.md, Route S): delta in one subsystem, ≤ ~15 tasks, no *novel* external integration (an integration already in ARCHITECTURE.md doesn't disqualify; a new well-known-API integration qualifies with a compact ≤1-page wire contract, verified-fake rule stands), delta stakes low → the cycle takes `profile: lite` regardless of the app's own profile. ARCHITECTURE.md and UX.md are patched (Opus-tier), never regenerated. Feature branch. |
