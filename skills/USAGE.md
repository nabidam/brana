...
**Gate crystallization is coverage-first:** compare each gate journey with the existing journey suite. Full coverage → rerun and record serving test paths; partial coverage → extend only for uncovered behavior; no suitable test → create one. New gate numbers never require duplicate tests. Skipped gates run the same coverage check immediately; only the human walkthrough remains `UNWITNESSED`.
