...
Seven skills implementing Brana, the 6-phase app-dev workflow + Phase 7 change loop (WORKFLOW.md v1.12). Each skill has **run** mode (executes in Claude Code, Agent Adaptation Layer applies) and **prompt** mode (emits paste-ready blocks for external chat UIs — copy-paste is the workflow's canon medium).
...
**Gate crystallization is coverage-first:** compare each gate journey with the existing journey suite. Full coverage → rerun and record serving test paths; partial coverage → extend only for uncovered behavior; no suitable test → create one. New gate numbers never require duplicate tests — but the rotated unglamorous step must be named in the coverage. The gate's completion mark quotes the coverage test path(s) (`` coverage `tests/...` ``, WALKED and SKIPPED alike); `brana-gate` verifies each cited path exists. Skipped gates run the same coverage check immediately (evidence + coverage citations required); only the human walkthrough remains `UNWITNESSED`.

**v1 exit bar:** the release-gate task, preflighted like any gate (preflight includes a `brana-gate tasks` re-run — done-mark integrity: SHA + existing non-empty evidence file per Done mark, existing coverage test paths on merged-form gate marks — WALKED and SKIPPED alike — deps resolved in order) — kernel journey end-to-end in a release build through the production composition, witnessed, including restart/offline/error paths; cumulative journey-suite coverage for every kernel step green; production-composition proof + verified-fake suites green when fakes stood in; every PRD NFR budget measured at or under budget (or explicitly accepted over); every `GATE SKIPPED` listed with its `UNWITNESSED` walkthrough, walked now or explicitly accepted; unresolved `GATE BLOCKED` fails the bar. "All tasks Done" is not the bar. Blocked-gate fixes that reveal a missing subsystem spawn a child spec cycle (Spawn route), never a wedge into the current TASKS.md.
