...
| **External system** | a third-party API/service that will be faked in tests | wire contract in PLAN.md (exact shapes, auth, error semantics); verified fake: one shared contract suite runs against fake and real adapter; real side offline-assertable; live calls canary-only |
...
| **UI-heavy** | the product's value is its interface | UX flows per screen (regions, states: empty/loading/error/focus) in PLAN.md §4; DESIGN.md with semantic tokens, WCAG AA contrast; a11y check in verify |
| **Auth / user data** | login, stored personal data, external input | threat model section: trust boundaries, authZ per surface, input validation, secrets handling; security review on auth diffs |
...
4. Deterministic remainder: run `tools/brana-gate docs PLAN.md` (placeholder
   scan; contrast computation when DESIGN.md exists).
...
- **Dependency:** a needed capability outside the plan's list → propose, ask.
...
one line per unit: id, status, SHA, date. Ledger and evidence output are never
committed into PLAN.md and never generate their own commits; the ledger rides
along in the unit's commit. After compaction, state rebuilds from git log +
ledger, never from remembered conversation.
...
words generated, units, fix units, agent invocations, and (when available)
billed tokens. Compare across cycles before tightening or loosening any rule.
