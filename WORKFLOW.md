# Brana — App Development Workflow (v1.12)
...
### Architecture review (blocks Phase 3)

The consistency gate checks that the docs agree with each other; nothing checks that the design is any good — wrong-but-consistent architecture passes every machine pass and then every implementation token is spent building the wrong thing. One independent review, findings-only, before Phase 3. Reviewer independence rules are 6a's: the reviewer gets ARCHITECTURE.md + PRD.md + UX.md, never the author's rationale; fresh session, different model than the author where possible (cross-vendor preferred — a same-vendor fresh session is weaker independence, still better than none).
...
The human arbitrates the findings — this review judges intent-level trade-offs, so accepting a finding is a product decision, not a mechanical fix; ARCHITECTURE.md is patched before Phase 3 starts.
...
- Every DEMO GATE task **contains its crystallization coverage step** —
  ...
  duplicate test merely because it has a new N — but the unglamorous
  step rotates across gates by design, so a "full coverage" claim must
  name the test serving *this* gate's rotated step, not last gate's.
  The completion mark quotes the coverage test path(s) — `` coverage
  `tests/...` `` — and `brana-gate` verifies each cited path exists:
  coverage is a citation, never an assertion. Do this in the gate
...
**Completing a task:** run the **verify script** (Verification Machinery) _and_ its acceptance behavior. UI tasks: launch-and-look + screenshot only if the human asked for it (per task or in CONVENTIONS.md) — otherwise visual quality waits for the demo gate. Then commit, capture the **evidence file** at `specs/NNN-name/evidence/task-N.txt` (the exercised command — verify script, test, or journey step — plus the last ~30 lines of its output), and mark the task done in TASKS.md with the commit SHA plus the evidence file's path — grammar `` - **Done:** `SHA` — evidence `specs/NNN-name/evidence/task-N.txt` `` (gate tasks: `` - **GATE n WALKED — PASS** (date, human) — evidence `...`. coverage `tests/...` `` — the coverage citation names the test file(s) serving the journey, reused, extended, or new; a `GATE n SKIPPED` mark carries the same evidence and coverage citations, since a skip defers only the walkthrough), which `brana-gate tasks` machine-checks: SHA present, evidence file exists non-empty, coverage paths exist on merged-form gate marks, deps resolved first. Evidence is captured live — it is not reconstructable from the diff afterward, and a done-mark without it doesn't pass the demo gate or the v1 exit bar. Marking Done on green tests alone is the classic failure — don't reintroduce it.
